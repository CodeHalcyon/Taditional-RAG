...
It loads a PDF, splits it into chunks, embeds it using a SentenceTransformer model, stores it in **ChromaDB**, retrieves relevant chunks for a query, and finally generates an enhanced response using **Google’s Gemini model**.
...
* Modular design with custom classes:
  ...
  * `EmbeddingManager` – for handling embeddings
  * `VectorStorage` – for ChromaDB operations
  * `ragretriver` – for context retrieval
  * `rag_res` – for generating LLM-based responses
...
project/
│
├── cc2.pdf                     # Your input PDF file
├── main.ipynb / script.py      # Main pipeline script
├── data/
│   └── vector_store/           # Persistent ChromaDB store
...
### 3️⃣ Install dependencies manually (if no requirements.txt)
...
### 6️⃣ Generate Final Answer via LLM
...
**Chetan Sirigiri**
📧 [chetandev0109@gmail.com](mailto:chetandev0109@gmail.com)
🌐 [LinkedIn](https://www.linkedin.com/in/chetan-sirigiri) | [GitHub](https://github.com/codehalcyon)
